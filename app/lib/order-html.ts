/**
 * order-html.ts
 * Generates A4 print HTML for orders — same design as invoice-html.ts.
 */
...
const DIVISION = {
  ...
  address: "Pranawatta Road, Wallabada, Boossa",
  tel: "0777681663",
};

export const generateOrderHTML = (order: any, items: any[]): string => {
  const shopName = order.customer?.shopName || order.customer?.name || "Unknown";
  const customerName = order.customer?.name || shopName;
  const address = order.customer?.address || "";
  const phone = order.customer?.phone || "";
  const route = order.customer?.route || "";
  ...
  const rows = items
    ...
    <tr style="${idx < items.length - 1 ? "border-bottom:1px solid #e8e8e8;" : ""}">
      ...
      <td style="padding:7px 8px;font-size:11px;color:#111;font-weight:600;vertical-align:middle;">${item.name || "-"}</td>
      <td style="padding:7px 8px;font-size:11px;color:#111;text-align:right;white-space:nowrap;vertical-align:middle;">LKR ${fmt(item.price)}</td>
      <td style="padding:7px 8px;font-size:11px;font-weight:700;color:#111;text-align:center;vertical-align:middle;">${item.qty}</td>
      <td style="padding:7px 8px;font-size:11px;color:#555;text-align:center;vertical-align:middle;">${item.unit || "Pcs"}</td>
      <td style="padding:7px 8px;font-size:11px;text-align:center;color:#111;vertical-align:middle;">${(item.free || 0) > 0 ? item.free : "-"}</td>
      <td style="padding:7px 8px;font-size:11px;text-align:center;color:#777;vertical-align:middle;">${(item.discountPercent || 0) > 0 ? "-" + item.discountPercent + "%" : "-"}</td>
      ...
      <tr>
        <td style="vertical-align:top;width:60%;">
          ...
          <div style="font-size:10px;color:#333;margin-top:2px;line-height:1.5;">
            ${DIVISION.division}<br>${DIVISION.address}<br>Tel: ${DIVISION.tel}
          </div>
        </td>
        <td style="vertical-align:top;text-align:right;width:40%;">
          <div style="font-size:18px;font-weight:700;color:#000;letter-spacing:0.5px;">${order.orderId || "-"}</div>
          <div style="display:inline-block;margin-top:4px;padding:2px 8px;border:1.5px solid #555;border-radius:4px;font-size:10px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:#333;">${order.status || "ORDER"}</div>
          ${order.invoiceNo ? `<div style="font-size:11px;color:#555;margin-top:4px;">Invoice: ${order.invoiceNo}</div>` : ""}
          <div style="font-size:11px;color:#333;margin-top:4px;line-height:1.5;">
            ${formatDate(order.date || order.createdAt)}<br>${salesRep}
          </div>
        </td>
      </tr>
    </table>
...
export const printOrder = (order: any, items: any[]) => {
  const html = generateOrderHTML(order, items);
  printHTML(getDocumentWrapper(html, order.orderId || "Order"));
};
