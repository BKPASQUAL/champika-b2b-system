...
export default function InventoryPage() {
  ...
  const handleReconcileNegative = async () => {
    if (
      !confirm(
        `Are you sure you want to reset all ${negativeStockCount} negative stock item(s) to 0 across the entire system?\n\nThis will adjust all negative stock quantities to 0 and record audit logs.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/inventory/reconcile-negative", {
        ...
        body: JSON.stringify({
          reason: "Bulk reconciliation of negative stock to 0 via Admin Overview",
        }),
      });
                      ...
                      {loc.isDamageLocation ? (
                        ...
                        <div className="flex justify-end gap-1.5">
                          <Button
                            ...
                            onClick={() =>
                              router.push(`/dashboard/admin/inventory/${loc.id}/adjust`)
                            }
                          >
                            Adjust
                          </Button>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row justify-between gap-4 items-center">
              <div>
                ...
                <CardDescription>
                  Detailed stock levels including damaged items. Click any filter below for 1-click status view.
                </CardDescription>
              </div>
