...
function EventPill({
  ...
  return (
    <button
      ...
      className={`w-full text-left rounded px-1.5 py-0.5 text-xs font-medium truncate transition-colors ${
        isCustomer ? "bg-blue-500 hover:bg-blue-600 text-white" : "bg-orange-500 hover:bg-orange-600 text-white"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-0.5">{isCustomer ? "Customer" : "Supplier"}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold">LKR {Number(event.amount).toLocaleString("en-LK")}</p>
            </div>
            <div>
            ...
            <div>
              <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-0.5">{isCustomer ? "Invoice" : "Payment Date"}</p>
...
function MonthView({
  ...
  while (cells.length < 42) cells.push({ date: new Date(year, month + 1, cells.length - daysInMonth - firstDay + 1), inMonth: false });
          ...
          return (
            <div key={idx} className={`min-h-[90px] p-1 flex flex-col gap-0.5 ${!inMonth ? "bg-gray-50" : ""}`}>
              <div className="flex items-center justify-between mb-0.5">
                ...
                >{date.getDate()}</button>
                {events.length > 0 && <span className="text-[10px] text-muted-foreground">{events.length}</span>}
              </div>
              <div className="space-y-0.5 overflow-hidden">
                {events.slice(0, 3).map((ev) => <EventPill key={ev.id} event={ev} onClick={onEventClick} compact />)}
                {events.length > 3 && (
...
function WeekView({
  weekStart, eventsByDate, onEventClick, onDayClick,
}: {
  weekStart: Date;
  eventsByDate: Map<string, ChequeEvent[]>;
  onEventClick: (e: ChequeEvent) => void;
        ...
        {days.map((d) => {
          ...
          return (
            <div key={key} className="text-center py-2 border-r last:border-r-0">
              ...
              >{d.getDate()}</button>
              {evCount > 0 && <p className="text-[10px] text-muted-foreground mt-0.5">{evCount} cheque{evCount !== 1 ? "s" : ""}</p>}
            </div>
          );
        })}
      </div>
        ...
        {days.map((d) => {
          ...
          return (
            <div key={key} className="p-1.5 space-y-1 min-h-[300px]">
              {events.length === 0 && <p className="text-[10px] text-gray-300 text-center mt-4">—</p>}
              {events.map((ev) => <EventPill key={ev.id} event={ev} onClick={onEventClick} />)}
            </div>
          );
        })}
      </div>
...
function DayView({
  date, eventsByDate, onEventClick,
}: {
  date: Date;
  eventsByDate: Map<string, ChequeEvent[]>;
  onEventClick: (e: ChequeEvent) => void;
    ...
    <div className="flex flex-col flex-1 overflow-auto">
      <div className={`px-6 py-4 border-b ${isToday ? "bg-blue-50" : "bg-gray-50"}`}>
        <div className="flex items-baseline gap-3">
          <span className={`text-4xl font-bold ${isToday ? "text-blue-600" : "text-gray-800"}`}>{date.getDate()}</span>
          <div>
            <p className="text-sm font-semibold text-gray-700">{DAY_NAMES_FULL[date.getDay()]}, {MONTH_NAMES[date.getMonth()]} {date.getFullYear()}</p>
            ...
            <div className="ml-auto flex gap-4 text-right">
              {customerEvents.length > 0 && (
                <div>
                  ...
                  <p className="text-sm font-bold text-blue-600">{formatLKR(customerEvents.reduce((s, e) => s + e.amount, 0))}</p>
              ...
              {supplierEvents.length > 0 && (
                <div>
                  ...
                  <p className="text-sm font-bold text-orange-600">{formatLKR(supplierEvents.reduce((s, e) => s + e.amount, 0))}</p>
            ...
            {customerEvents.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Customer Cheques ({customerEvents.length})</p>
                </div>
                    ...
                    return (
                      <button key={ev.id} onClick={() => onEventClick(ev)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">Cheque #{ev.cheque_number || "—"} · Invoice {ev.reference}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold text-blue-700">LKR {Number(ev.amount).toLocaleString("en-LK")}</p>
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${colors.badge}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${colors.dot}`} />{ev.status || "Pending"}
                          </span>
                        </div>
                      </button>
                    );
            ...
            {supplierEvents.length > 0 && (
              <div className={customerEvents.length > 0 ? "mt-4" : ""}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Supplier Cheques ({supplierEvents.length})</p>
                </div>
                <div className="space-y-2">
                  {supplierEvents.map((ev) => {
                    ...
                    return (
                      <button key={ev.id} onClick={() => onEventClick(ev)}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-800 truncate">{ev.name}</p>
                          <p className="text-xs text-muted-foreground">Cheque #{ev.cheque_number || "—"} · Payment date {ev.reference}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold text-orange-700">LKR {Number(ev.amount).toLocaleString("en-LK")}</p>
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${colors.badge}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${colors.dot}`} />{ev.status || "Pending"}
                          </span>
                        </div>
    ...
    <div className="flex flex-col h-full space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold tracking-tight">Cheque Calendar</h2>
        </div>

        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-3 text-xs mr-2">
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-blue-500 inline-block" />Customer</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-orange-500 inline-block" />Supplier</span>
          </div>
          <div className="flex rounded-lg border overflow-hidden text-xs">
            {(["all", "customer", "supplier"] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 capitalize font-medium transition-colors ${filter === f ? "bg-gray-800 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>
                {f}
              </button>
            ))}
          </div>
          <div className="flex rounded-lg border overflow-hidden text-xs">
            {(["month", "week", "day"] as const).map((v) => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 capitalize font-medium transition-colors ${view === v ? "bg-blue-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>
                {v}
              </button>
            ))}
          </div>
      ...
      <div className="flex flex-col flex-1 border rounded-xl bg-white overflow-hidden min-h-[500px]">
        <div className="flex items-center justify-between px-4 py-3 border-b bg-gray-50">
          <button onClick={goToday} className="text-xs font-medium px-3 py-1.5 rounded-lg border bg-white hover:bg-gray-50 transition-colors">Today</button>
          <div className="flex items-center gap-3">
            <button onClick={prev} className="p-1.5 rounded-lg hover:bg-gray-200 transition-colors"><ChevronLeft className="h-4 w-4" /></button>
            <h3 className="text-sm font-semibold min-w-[220px] text-center">{title}</h3>
            <button onClick={next} className="p-1.5 rounded-lg hover:bg-gray-200 transition-colors"><ChevronRight className="h-4 w-4" /></button>
          </div>
          <div className="w-16" />
        </div>
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center gap-2 text-muted-foreground">
            <Loader2 className="h-5 w-5 animate-spin" /><span className="text-sm">Loading cheques...</span>
          </div>
        ) : error ? (
          ...
          <MonthView year={year} month={month} eventsByDate={eventsByDate} onEventClick={setSelected} onDayClick={goToDay} />
        ) : view === "week" ? (
          <WeekView weekStart={weekStart} eventsByDate={eventsByDate} onEventClick={setSelected} onDayClick={goToDay} />
        ) : (
          <DayView date={currentDate} eventsByDate={eventsByDate} onEventClick={setSelected} />
        )}
      </div>
