...
export default function WiremanReportPage() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch Wireman report");
      const json = await res.json();
      setData(json);
    } catch (err) {
      console.error(err);
      toast.error("Failed to load Wireman report");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-5 p-2 sm:p-4 lg:p-6">
      {/* ── Page Title ──────────────────────────────────────────────────────── */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Wireman Agency</h1>
                  ...
                  onClick={() => {
                    ...
                    import("jspdf").then(({ jsPDF }) => {
                      import("jspdf-autotable").then(() => {
                        const doc = new jsPDF();
                        doc.setFontSize(18);
                        doc.text("Wireman Agency — Profit Report", 14, 20);
                        doc.setFontSize(11);
                        ...
                        doc.save(`wireman-profit-report-${customFrom}-to-${customTo}.pdf`);
                      });
                    });
                  }}
                  ...
                  <CardContent className="space-y-5 pt-2">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">LKR {fmt(overview.totalSales)}</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-red-600">LKR {fmt(overview.totalCost)}</p>
                      </div>
                      ...
                      {(data?.orders ?? []).length === 0 ? (
                        ...
                        (data?.orders ?? []).map((o: any) => (
                          <TableRow key={o.id}>
                            ...
                            <TableCell className="text-right font-bold">{fmt(o.amount)}</TableCell>
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Wireman Agency operational expenses
                    </p>
                  </div>
                      ...
                      {(data?.dueInvoices ?? []).length === 0 ? (
                        ...
                        (data?.dueInvoices ?? []).map((d: any) => (
                          <TableRow key={d.id}>
                            ...
                            <TableCell className="text-right font-bold">{fmt(d.amount)}</TableCell>
                            <TableCell className="text-right font-bold text-red-600">
                              {fmt(d.dueAmount)}
                            </TableCell>
                            <TableCell>
                  ...
                  <CardContent className="space-y-3">
                    {[
                      ...
                      <div key={row.label} className="flex items-center justify-between py-1.5 border-b last:border-0">
                        ...
                        <span className={`text-sm font-bold ${row.color}`}>LKR {fmt(row.value)}</span>
                      </div>
                    ))}
                  </CardContent>
                </Card>
