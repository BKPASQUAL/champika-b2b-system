...
export default function ProductDetailsPage({
  ...
  const handleViewTransaction = (tx: Transaction) => {
    ...
    let prefix = "/dashboard/admin";
    if (currentPath.includes("/office/distribution")) prefix = "/dashboard/office/distribution";
                    ...
                    <div key={transaction.id} className="border rounded-lg p-3 space-y-2 bg-card">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 text-xs pt-1 border-t">
                        ...
                        {transaction.invoiceNo && (
                          <button
                            onClick={() => {
                              if (transaction.invoiceId) router.push(`/dashboard/admin/invoices/${transaction.invoiceId}?fromProduct=${id}`);
                        ...
                        {transaction.orderId && (
                          <button
                            onClick={() => {
                              if (transaction.orderDbId || transaction.orderId) router.push(`/dashboard/admin/orders/${transaction.orderDbId || transaction.orderId}?fromProduct=${id}`);
                  ...
                  {paginatedTransactions.length === 0 ? (
                    ...
                    paginatedTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                              ...
                              <button
                                onClick={() => {
                                  if (transaction.invoiceId) {
                                    router.push(`/dashboard/admin/invoices/${transaction.invoiceId}?fromProduct=${id}`);
                                  } else {
                              ...
                              <button
                                onClick={() => {
                                  if (transaction.orderDbId || transaction.orderId) {
                                    router.push(`/dashboard/admin/orders/${transaction.orderDbId || transaction.orderId}?fromProduct=${id}`);
                                  } else {
                        ...
                        <TableCell className="text-center">
                          <span className="text-xs font-medium text-muted-foreground truncate max-w-[110px] block" title={transaction.businessName}>
                            {transaction.businessName || "-"}
                          </span>
