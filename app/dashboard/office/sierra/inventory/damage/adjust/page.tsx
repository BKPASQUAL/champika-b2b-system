...
export default function SierraDamageAdjustmentPage() {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/settings/locations?businessId=${BUSINESS_IDS.SIERRA_AGENCY}`);
    ...
    const fetchStocks = async () => {
      try {
        ...
        const [productsRes, stockRes] = await Promise.all([
          ...
          fetch(`/api/inventory/${selectedLocationId}?businessId=${BUSINESS_IDS.SIERRA_AGENCY}&includeAll=true`),
        ]);
    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => router.push("/dashboard/office/sierra/inventory/damage")}
            >
            ...
            <h1 className="text-3xl font-bold tracking-tight text-purple-950 flex items-center gap-2">
              ...
              Adjust Damaged Stock (Sierra)
            </h1>
          </div>
          <p className="text-muted-foreground mt-1 ml-8">
            Perform physical audit, write-offs, and count corrections for Sierra damaged items.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-1.5 text-slate-700">
                <MapPin className="w-4 h-4 text-purple-600" /> Target Location
              </label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-1.5 text-slate-700">
                <Layers className="w-4 h-4 text-purple-600" /> Batch Adjustment Reason
              </label>
              <Input
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <CardContent className="space-y-4">
            {stockLoading ? (
              ...
              <>
                ...
                {/* Product Snapshot */}
                <div className="rounded-lg border bg-purple-50/40 p-3 flex gap-3 items-start">
                  <div className="shrink-0">
                    {selectedProduct?.images?.[0] ? (
                      <img
                        src={selectedProduct.images[0]}
                        alt={selectedProduct?.name}
                        className="w-16 h-16 rounded-lg object-cover border border-purple-200 shadow-sm"
                      />
                    ) : (
                      <div className="w-16 h-16 rounded-lg border border-purple-200 bg-white flex items-center justify-center">
                ...
                <div className="flex rounded-md border overflow-hidden text-xs font-semibold">
                  <button
                    ...
                    className={cn(
                      ...
                      addMode === "set" ? "bg-purple-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70"
                    )}
                  ...
                  <button
                    ...
                    className={cn(
                      "flex-1 py-2 transition-colors",
                      addMode === "add" ? "bg-purple-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70"
                    )}
                  ...
                  <button
                    ...
                    className={cn(
                      "flex-1 py-2 transition-colors",
                      addMode === "subtract" ? "bg-purple-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70"
                    )}
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent>
            {pendingAdjustments.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground border-2 border-dashed rounded-lg bg-purple-50/20">
                <AlertCircle className="w-10 h-10 mb-2 opacity-30 text-purple-500" />
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            ...
            <CardDescription>
              All Sierra products that currently have non-zero damaged units in this warehouse.
            </CardDescription>
          </div>
