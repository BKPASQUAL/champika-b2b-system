...
// Note: Ensure this component exists or create it for Sierra if needed
// import { LocationSettingsSheet } from "./_components/LocationSettingsSheet";

export default function SierraLocationInventoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        // ✅ Passed BUSINESS_IDS.SIERRA_AGENCY to filter results on the server
  ...
  const generatePDF = (items: any[], title: string, filename: string) => {
    ...
    autoTable(doc, {
      ...
      body: tableRows,
      startY: 35,
      headStyles: { fillColor: [220, 38, 38] }, // Red header for Sierra
    });
  ...
  const handleExportAll = (type: "pdf" | "excel") => {
    ...
    if (type === "pdf")
      generatePDF(
        items,
        "Location Inventory Report (Sierra)",
        "Inventory_Report",
      );
  ...
  const handleExportDamage = (type: "pdf" | "excel") => {
    ...
    if (type === "pdf")
      generatePDF(items, "Damage Stock Report (Sierra)", "Damage_Report");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {/* Navigate to Sierra Adjust Page */}
          <Button
            variant="default"
            size="sm"
            onClick={() =>
              router.push(
                `/dashboard/office/sierra/inventory/${locationId}/adjust`,
              )
            }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-green-500 shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Sierra stock worth</p>
          </CardContent>
        </Card>
              ...
              <TableBody>
                {filteredStocks.length === 0 ? (
                  <TableRow>
                    ...
                    >
                      No Sierra stock found at this location.
                    </TableCell>
                  </TableRow>
                ) : (
