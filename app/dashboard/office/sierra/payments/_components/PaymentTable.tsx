// app/dashboard/office/sierra/payments/_components/PaymentTable.tsx
"use client";
...
export function PaymentTable({
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortOrder === "asc"
      ? <ArrowUp className="w-3.5 h-3.5 ml-1 text-purple-600" />
      : <ArrowDown className="w-3.5 h-3.5 ml-1 text-purple-600" />;
  };
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50 hover:bg-gray-50">
              <TableHead className="cursor-pointer text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 px-4 whitespace-nowrap" onClick={() => onSort("date")}>
              ...
              <TableHead className="cursor-pointer text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 px-4" onClick={() => onSort("customerName")}>
                <div className="flex items-center">Customer {getSortIcon("customerName")}</div>
              </TableHead>
              <TableHead className="cursor-pointer text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 px-4 text-right" onClick={() => onSort("amount")}>
                <div className="flex items-center justify-end">Amount {getSortIcon("amount")}</div>
              </TableHead>
              <TableHead className="cursor-pointer text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 px-4" onClick={() => onSort("method")}>
              ...
              <TableHead className="cursor-pointer text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 px-4" onClick={() => onSort("chequeStatus")}>
          ...
          <TableBody>
            {payments.length === 0 ? (
              ...
              payments.map((payment) => {
                ...
                const dateStr = payment.date ? new Date(payment.date).toLocaleDateString("en-LK") : "—";
                const chequeDateStr = payment.chequeDate ? new Date(payment.chequeDate).toLocaleDateString("en-LK") : null;

                return (
                  <TableRow key={payment.id} className={`transition-colors border-b border-gray-100 last:border-0 ${payment.isCancelled ? "bg-red-50/50 hover:bg-red-50/70 opacity-75" : payment.chequeStatus === "Returned" ? "bg-red-50/40 hover:bg-red-50/60" : "hover:bg-purple-50/20"}`}>
                    ...
                    <TableCell className="py-3 px-4">
                      <span className="text-sm font-semibold text-gray-800">{payment.invoiceNo}</span>
                    </TableCell>
                    <TableCell className="py-3 px-4">
                      <button
                        onClick={() => window.open(`/dashboard/office/sierra/invoices/${payment.invoiceId}`, "_blank")}
                        className="text-sm font-medium text-purple-600 hover:underline text-left"
                      >
                    ...
                    <TableCell className="py-3 px-4">
                      {payment.depositAccountName ? (
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-gray-800">{payment.depositAccountName}</span>
                          {payment.depositAccountType && (
                            <span className="text-xs text-muted-foreground">{payment.depositAccountType}</span>
                          )}
                        </div>
                      ) : (
                    ...
                    <TableCell className="py-3 px-4">
                      {isCheque ? (
                        <div className="flex flex-col gap-0.5">
                          <span className="text-sm font-medium text-gray-800">Cheque: {payment.chequeNo || "—"}</span>
                          {(payment.bankCode || payment.bankName) && (
                            <span className="text-xs text-muted-foreground">{[payment.bankCode, payment.bankName].filter(Boolean).join(" - ")}</span>
                          )}
                          {chequeDateStr && <span className="text-xs text-muted-foreground">Date: {chequeDateStr}</span>}
                        </div>
                      ) : (
                    ...
                    <TableCell className="py-3 px-4">
                      {isCheque ? <ChequeStatusBadge status={payment.chequeStatus} /> : <span className="text-xs text-muted-foreground">—</span>}
                    </TableCell>
                    <TableCell className="py-3 px-4">
                      <DropdownMenu>
