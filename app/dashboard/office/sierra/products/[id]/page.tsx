...
export default function SierraProductDetailsPage({
  params,
}: {
    ...
    const fetchData = async () => {
      try {
        ...
        const prodRes = await fetch(
          `/api/products/${id}?businessId=${BUSINESS_IDS.SIERRA_AGENCY}`,
        );

        if (prodRes.status === 404) {
          toast.error("Product not found in Sierra catalog");
          router.push("/dashboard/office/sierra/products");
          return;
        }
  ...
  const handleViewTransaction = (tx: Transaction) => {
    ...
    let prefix = "/dashboard/office/sierra";
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                All transactions (Sales, Purchases, Returns) for Sierra
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Mobile Card View */}
                    ...
                    <div key={transaction.id} className="border rounded-lg p-3 space-y-2 bg-card">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 text-xs pt-1 border-t">
                        ...
                        {transaction.invoiceNo && (
                          <button
                            onClick={() => {
                              if (transaction.invoiceId) router.push(`/dashboard/office/sierra/invoices/${transaction.invoiceId}?fromProduct=${id}`);
                        ...
                        {transaction.orderId && (
                          <button
                            onClick={() => {
                              if (transaction.orderDbId || transaction.orderId) router.push(`/dashboard/office/sierra/orders/${transaction.orderDbId || transaction.orderId}?fromProduct=${id}`);
                              ...
                              <button
                                onClick={() => {
                                  if (transaction.invoiceId) {
                                    router.push(`/dashboard/office/sierra/invoices/${transaction.invoiceId}?fromProduct=${id}`);
                                  } else {
                              ...
                              <button
                                onClick={() => {
                                  if (transaction.orderDbId || transaction.orderId) {
                                    router.push(`/dashboard/office/sierra/orders/${transaction.orderDbId || transaction.orderId}?fromProduct=${id}`);
                                  } else {
