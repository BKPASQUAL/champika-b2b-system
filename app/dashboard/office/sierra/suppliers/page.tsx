...
// Ensure you copy these components from admin if they don't exist in your directory yet
...
export default function SierraSuppliersPage() {
  ...
  const [formData, setFormData] = useState<SupplierFormData>({
    ...
    status: "Active",
    duePayment: 0,
    // ✅ Automatically assign Sierra Business ID
    businessId: BUSINESS_IDS.SIERRA_AGENCY,
  });

  // --- 1. Fetch Suppliers (Filtered for Sierra) ---
  const fetchSuppliers = useCallback(async () => {
    try {
      setLoading(true);
      // ✅ Pass businessId param
      const response = await fetch(
        `/api/suppliers?businessId=${BUSINESS_IDS.SIERRA_AGENCY}`,
      );
  ...
  const generateExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Sierra_Suppliers_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  };
  ...
  const generatePDF = () => {
    ...
    const doc = new jsPDF();
    doc.text("Sierra Suppliers Report", 14, 15);
    autoTable(doc, {
    ...
    doc.save(`Sierra_Suppliers_${new Date().toISOString().split("T")[0]}.pdf`);
  };
          ...
          <SupplierTable
            ...
            onEdit={(s) => {
              setFormData({
                ...
                businessId: s.businessId || BUSINESS_IDS.SIERRA_AGENCY,
              });
