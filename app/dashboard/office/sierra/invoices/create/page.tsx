...
export default function CreateSierraInvoicePage() {
  ...
  const [autoInvoiceNo, setAutoInvoiceNo] = useState<string>("");
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Fetch Next Auto Invoice Number (SI-xxxx)
        try {
          const invRes = await fetch(`/api/invoices/next-number?businessId=${BUSINESS_IDS.SIERRA_AGENCY}`);
          if (invRes.ok) {
            const invJson = await invRes.json();
            if (invJson.invoiceNo) setAutoInvoiceNo(invJson.invoiceNo);
          }
        } catch (err) {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-medium">System Invoice # (Auto)</Label>
                  <Input
                    value={autoInvoiceNo || "Auto-Generated (SI-…)"}
                    disabled
                    className="bg-muted font-mono font-bold text-slate-800"
                  />
                  <p className="text-xs text-muted-foreground">Auto sequence generated by Sierra system</p>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
