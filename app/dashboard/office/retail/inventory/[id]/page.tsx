...
export default function RetailLocationInventoryPage() {
  ...
  const handleExportAll = (type: "pdf" | "excel") => {
    ...
    if (type === "pdf") generatePDF(items, "Retail Location Inventory Report", "Retail_Inventory");
    else generateExcel(items, "Inventory", "Retail_Inventory");
  };

  const handleExportDamage = (type: "pdf" | "excel") => {
    const items = data?.stocks.filter((s: any) => (s.damagedQuantity || 0) > 0) || [];
    if (type === "pdf") generatePDF(items, "Retail Damage Stock Report", "Retail_Damage");
    else generateExcel(items, "Damaged Items", "Retail_Damage");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push("/dashboard/office/retail/inventory")}>
        ...
        <div className="flex gap-2 flex-wrap">
          <Button
            variant="default"
            size="sm"
            onClick={() => router.push(`/dashboard/office/retail/inventory/${locationId}/adjust`)}
          ...
          <div className="flex items-center gap-1 bg-muted/20 p-1 rounded-lg border">
            <Button variant="ghost" size="sm" onClick={() => handleExportAll("pdf")}>
              <FileDown className="w-4 h-4 mr-2" /> PDF
            </Button>
            <Button variant="ghost" size="sm" onClick={() => handleExportAll("excel")}>
                ...
                {filteredStocks.length === 0 ? (
                  ...
                  filteredStocks.map((stock: any) => (
                    <TableRow key={stock.id}>
                      <TableCell className="font-mono text-xs">{stock.sku}</TableCell>
                      ...
                      <TableCell className="text-right font-bold text-red-600">
                        {stock.damagedQuantity > 0 ? stock.damagedQuantity : "-"}
                      </TableCell>
