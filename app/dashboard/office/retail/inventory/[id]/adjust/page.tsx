...
export default function RetailStockAdjustmentPage() {
  ...
  const [editDialog, setEditDialog] = useState<{
    open: boolean; productId: string; productName: string; editValue: string;
  }>({ open: false, productId: "", productName: "", editValue: "" });

  const [renameDialog, setRenameDialog] = useState<{
    open: boolean; productId: string; currentName: string; newName: string; saving: boolean;
    ...
    const initData = async () => {
      try {
        ...
        const [productsRes, stockRes, packRes] = await Promise.all([
          fetch(`/api/inventory?businessId=${BUSINESS_IDS.CHAMPIKA_RETAIL}`),
          fetch(`/api/inventory/${locationId}?businessId=${BUSINESS_IDS.CHAMPIKA_RETAIL}&includeAll=true`),
  ...
  const handleAddToTable = () => {
    ...
    if (pendingAdjustments.some((p) => p.productId === selectedProductId)) {
      return toast.error("This product is already in the adjustment list");
    }
  ...
  const unlistedWithStock = allProducts.filter(
    (p) => (locationStocks[p.id] || 0) > 0 && !pendingAdjustments.some((a) => a.productId === p.id)
  );
  ...
  const handleSaveAll = () => {
    ...
    showConfirm("Confirm Adjustments", confirmMsg, async () => {
      ...
      try {
        ...
        if (zeroOutUnlisted) {
          unlistedWithStock.forEach((p) => itemsPayload.push({ productId: p.id, newQuantity: 0 }));
        }
        const res = await fetch("/api/inventory/adjust", {
          ...
          body: JSON.stringify({
            ...
            reason: zeroOutUnlisted
              ? "Full Stock Count — unlisted items zeroed (Retail Portal)"
              : "Manual Stock Adjustment (Retail Portal)",
        ...
        router.push(`/dashboard/office/retail/inventory/${locationId}`);
      } catch (error: any) {
        ...
        try {
          ...
          const res = await fetch("/api/inventory/adjust", {
            ...
            body: JSON.stringify({
              locationId,
              items: itemsPayload,
              reason: "Fresh Count Reset — all stock zeroed before manual count (Retail Portal)",
              businessId: BUSINESS_IDS.CHAMPIKA_RETAIL,
            }),
          });
          const result = await res.json();
    ...
    <div className="mx-auto space-y-6">
        ...
        <Card className="md:col-span-1 h-fit">
          ...
          <CardContent className="space-y-4">
                ...
                <div className="flex items-start gap-1">
                  ...
                  {selectedProduct && (
                    <button
                      ...
                      className="shrink-0 text-muted-foreground hover:text-green-600 transition-colors mt-0.5"
                    >
                ...
                {selectedProduct && ((selectedProduct.mrp ?? 0) > 0 || (selectedProduct.sellingPrice ?? 0) > 0) && (
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 pt-1 border-t">
                    ...
                    {(selectedProduct.sellingPrice ?? 0) > 0 && (
                      <span className="text-xs text-muted-foreground">Price: <span className="font-semibold text-green-700">Rs {selectedProduct.sellingPrice?.toLocaleString()}</span></span>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex rounded-md border overflow-hidden text-sm font-medium">
              <button
                type="button"
                className={cn("flex-1 py-2 transition-colors", addMode === "set" ? "bg-green-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70")}
              ...
              <button
                type="button"
                className={cn("flex-1 py-2 transition-colors", addMode === "add" ? "bg-green-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70")}
                  ...
                  <TableBody>
                    {pendingAdjustments.map((item) => (
                      <TableRow key={item.productId}>
                            ...
                            <div>
                              ...
                              {item.unitOfMeasure && (
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-xs text-green-600 font-medium">{item.unitOfMeasure}</span>
                        ...
                        <TableCell className="text-right font-bold text-green-600">{item.newStock}</TableCell>
                        <TableCell className="text-right">
                          <Badge
                            variant="outline"
                            className={item.difference < 0 ? "text-red-600 border-red-200 bg-red-50" : "text-green-600 border-green-200 bg-green-50"}
                          >
            ...
            <div className="mt-6 rounded-lg border border-green-200 bg-green-50 p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="zero-unlisted-retail" className="font-semibold text-green-800 cursor-pointer">
                    Zero out all unlisted products
                  </Label>
                </div>
                <Switch
                  id="zero-unlisted-retail"
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Tag className="w-4 h-4 text-green-600" /> Rename Product
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={handleRenameSave} disabled={renameDialog.saving}>
      ...
      <Dialog open={editDialog.open} onOpenChange={(open) => setEditDialog((d) => ({ ...d, open }))}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Pencil className="w-4 h-4 text-green-600" /> Edit Stock Quantity
            </DialogTitle>
            <DialogDescription>{editDialog.productName}</DialogDescription>
          </DialogHeader>
          <div className="py-2">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={handleEditSave}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
