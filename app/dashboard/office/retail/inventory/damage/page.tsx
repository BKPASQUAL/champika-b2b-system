...
export default function RetailDamageHistoryPage() {
  const router = useRouter();
  const { data: damages = [], loading, refetch: fetchDamages } = useCachedFetch<any[]>(
    `/api/inventory/damage?businessId=${BUSINESS_IDS.CHAMPIKA_RETAIL}`,
    [],
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Champika Retail - Damage Stock Report", 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`Retail_Damage_Report_${date}.pdf`);
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, `Retail_Damage_Report_${new Date().toLocaleDateString()}.xlsx`);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => router.push("/dashboard/office/retail/stock")}
              className="-ml-2 text-muted-foreground"
            >
              <ArrowLeft className="w-4 h-4" />
            </Button>
            <h1 className="text-3xl font-bold tracking-tight text-green-950">
              Damage Reports (Retail)
            </h1>
          </div>
          <p className="text-muted-foreground mt-1 ml-8">
            History and stock value of internal damages for Champika Retail.
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() => router.push("/dashboard/office/retail/inventory/damage/create")}
          >
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No damage reports found for Champika Retail.</p>
            </div>
          ) : (
            <>
                    ...
                    {paginatedDamages.map((item) => {
                      ...
                      return (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="text-right font-medium text-green-950">
