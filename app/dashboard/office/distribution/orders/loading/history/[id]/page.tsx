...
export default function DistributionLoadingSheetDetailPage({
  ...
  const [editForm, setEditForm] = useState({ lorryNumber: "", driverId: "", helperName: "", loadingDate: "", status: "" });
  ...
  const fetchDetails = async () => {
    try {
      ...
      setEditForm({ lorryNumber: data.lorryNumber, driverId: data.driverId, helperName: data.helperName || "", loadingDate: data.loadingDate, status: data.status });
    } catch (error) {
  ...
  const handlePrintAllInvoices = async () => {
    if (!loadingSheet?.orders.length) { toast.error("No orders to print"); return; }
    setPrintingInvoices(true);
    const ids = loadingSheet.orders.map((o) => o.invoiceId).filter((x): x is string => !!x);
    if (ids.length === 0) { toast.error("No invoice IDs found."); setPrintingInvoices(false); return; }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Delivered": return <Badge className="bg-green-600 hover:bg-green-700"><CheckCircle2 className="w-3 h-3 mr-1" /> Delivered</Badge>;
      case "Cancelled": return <Badge className="bg-red-600 hover:bg-red-700"><XCircle className="w-3 h-3 mr-1" /> Returned</Badge>;
      case "Loading": return <Badge className="bg-orange-500 hover:bg-orange-600"><RefreshCw className="w-3 h-3 mr-1" /> Rescheduled</Badge>;
      case "In Transit": return <Badge className="bg-blue-500 hover:bg-blue-600"><Send className="w-3 h-3 mr-1" /> In Transit</Badge>;
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {loadingSheet.status === "In Transit" && (
            <Button onClick={() => handleMoveStage("Completed")} disabled={stageMoveBusy} className="bg-green-600 hover:bg-green-700 text-white gap-2">
              {stageMoveBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
              Mark Completed
            </Button>
          )}
          {loadingSheet.status === "Completed" && (
            <Button onClick={() => handleMoveStage("In Transit")} disabled={stageMoveBusy} variant="outline" className="gap-2">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-blue-600">Final Value (Reconciled)</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-blue-700">Rs. {totalFinalValue.toLocaleString()}</div></CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Vehicle & Personnel</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-between border-b pb-2"><span className="text-muted-foreground">Vehicle</span><span className="font-medium">{loadingSheet.lorryNumber}</span></div>
            <div className="flex justify-between border-b pb-2"><span className="text-muted-foreground">Driver</span><span className="font-medium">{loadingSheet.driverName}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Helper</span><span className="font-medium">{loadingSheet.helperName || "-"}</span></div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="flex flex-col gap-2 items-center justify-center h-full py-2">
              <Badge className={`text-lg px-4 py-1 ${loadingSheet.status === "Completed" ? "bg-green-600" : "bg-blue-600"}`}>{loadingSheet.status}</Badge>
              <p className="text-sm text-muted-foreground">{loadingSheet.status === "Completed" ? "All items reconciled and closed." : "Currently in transit."}</p>
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  return (
                    <TableRow key={order.id}>
                      <TableCell className="font-medium font-mono">
                        ...
                        {order.invoiceId ? (
                          <button
                            onClick={() => window.open(`/dashboard/office/distribution/invoices/${order.invoiceId}`, "_blank")}
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><ShoppingBag className="h-5 w-5 text-indigo-600" />Add Orders to Sheet</DialogTitle>
            <DialogDescription>Select orders from the loading queue to add to this sheet.</DialogDescription>
          </DialogHeader>
          <Input placeholder="Search shop, invoice..." value={addSearch} onChange={(e) => setAddSearch(e.target.value)} className="mb-2" />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddOrders} disabled={addingBusy || selectedAddIds.length === 0} className="bg-indigo-600 hover:bg-indigo-700 text-white">
      ...
      <Dialog open={changeLorryOpen} onOpenChange={setChangeLorryOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Truck className="h-5 w-5 text-teal-600" />Change Lorry</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-64 overflow-y-auto py-2">
            {lorries.map((lorry) => {
              const isSelected = newLorry === lorry.name;
              return (
                <button key={lorry.id} onClick={() => setNewLorry(lorry.name)}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Edit Loading Sheet</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label>Lorry</Label>
              <Select value={editForm.lorryNumber} onValueChange={(val) => setEditForm({ ...editForm, lorryNumber: val })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{lorries.map((l) => <SelectItem key={l.id} value={l.name}>{l.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="grid gap-2">
              <Label>Driver</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateSheet} disabled={updating}>
