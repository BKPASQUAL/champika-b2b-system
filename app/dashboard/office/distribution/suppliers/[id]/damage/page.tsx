...
export default function SupplierDamagePage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      // Fetch with businessId to filter content
      const res = await fetch(
        `/api/suppliers/${supplierId}/damage?businessId=${businessId}`
      );
        ...
        <TableBody>
          {warehouseItems.length === 0 ? (
            <TableRow>
              ...
              >
                No damaged items found in Main Warehouse.
              </TableCell>
            </TableRow>
          ) : (
