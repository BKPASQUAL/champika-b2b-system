...
export default function DistributionDamageHistoryPage() {
  const router = useRouter();
  const { data: damages = [], loading, refetch: fetchDamages } = useCachedFetch<any[]>(
    `/api/inventory/damage?businessId=${BUSINESS_IDS.CHAMPIKA_DISTRIBUTION}`,
    [],
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Champika Distribution - Damage Stock Report", 14, 15);
    doc.setFontSize(10);
    ...
    doc.save(`Distribution_Damage_Report_${date}.pdf`);
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, `Distribution_Damage_Report_${new Date().toLocaleDateString()}.xlsx`);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight">
              Damage Reports (Distribution)
            </h1>
          </div>
          <p className="text-muted-foreground mt-1 ml-8">
            History and stock value of internal damages across all distribution stock.
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() =>
              router.push("/dashboard/office/distribution/inventory/damage/adjust")
            }
          >
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No damage reports found for Distribution.</p>
            </div>
          ) : (
            <>
