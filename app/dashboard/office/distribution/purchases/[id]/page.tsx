...
export default function DistributionViewPurchasePage({
  params,
}: {
  ...
  // 1. Items Subtotal (Sum of Unit Cost * Quantity, BEFORE discount)
  const itemsSubtotal = purchase.items.reduce(
    (acc, item) => acc + item.unitCost * item.quantity,
    0
  );

  // 2. Total Discount (Sum of all item discounts)
  const totalDiscount = purchase.items.reduce(
    (acc, item) => acc + (item.discount || 0),
    0
  );

  // 3. Gross Subtotal (Usually same as Items Subtotal in this context)
  const subtotalGross = itemsSubtotal;

  // 4. Net Invoice Amount (Total from DB)
  const netInvoiceAmount = purchase.totalAmount;

  // 5. Balance Due
  const balanceDue = netInvoiceAmount - purchase.paidAmount;
                    ...
                    <TableRow>
                      ...
                      <TableCell colSpan={3} className="text-right pr-6">
                        ...
                        <span className="font-bold font-mono text-base">
                          LKR{" "}
                          {itemsSubtotal.toLocaleString("en-LK", {
                            minimumFractionDigits: 2,
                          })}
                        </span>
                      </TableCell>
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-sm border-t-4 border-t-primary sticky top-6">
              ...
              <CardContent className="space-y-4 pt-6">
                ...
                {/* 2. Extra Discount (Sum of Item Discounts) */}
                {totalDiscount > 0 && (
                  <div className="flex justify-between text-sm text-red-600">
                    ...
                    <span className="font-medium font-mono">
                      - LKR{" "}
                      {totalDiscount.toLocaleString("en-LK", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
                ...
                {/* 3. Subtotal (Gross) - Display logic typically shows pre-tax/pre-final adjustments, here it's essentially same as Items Subtotal unless we separate them differently */}
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold font-mono text-slate-700">
                    LKR{" "}
                    {netInvoiceAmount.toLocaleString("en-LK", {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                </div>
