...
export default function SupplierDamagePage() {
  ...
  const generateReturnNote = (batch: any) => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text(`RETURN NOTE: ${supplier.name} (Orange Agency)`, 14, 20);
    doc.setFontSize(10);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        {/* Navigation back to ORANGE supplier details */}
        <Button
          variant="ghost"
          size="icon"
          onClick={() =>
            router.push(`/dashboard/office/orange/suppliers/${supplierId}`)
          }
        >
