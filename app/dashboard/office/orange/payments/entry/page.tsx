...
function StepBadge({ step, label }: { step: number; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-orange-500 text-white text-xs font-bold flex-shrink-0">{step}</div>
      <span className="text-sm font-medium text-muted-foreground">{label}</span>
    </div>
  );
}

export default function OrangePaymentEntryPage() {
  ...
  const availableAccounts = companyAccounts.filter((acc) => {
    ...
    if (paymentMethod === "cash") return t === "cash" || t === "cash on hand" || t === "wallet";
    if (paymentMethod === "bank") return t === "bank" || t === "savings" || t === "current";
    return false;
  });
        ...
        const filtered: PendingInvoice[] = invoicesData
          ...
          .map((inv: any) => {
            ...
            return {
              ...
              manualInvoiceNo: inv.manualInvoiceNo || undefined,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-orange-900">Payment Entry</h1>
            ...
            {paymentMethod === "cheque" && (
              <>
                <div className="space-y-2">
                  ...
                  <Input placeholder="e.g. 000123" value={chequeNumber} onChange={(e) => setChequeNumber(e.target.value)} />
                </div>
                <div className="space-y-2">
      ...
      <Card className="border-orange-100">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {selectedCustomerId && pendingInvoices.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <Download className="w-3.5 h-3.5 mr-1 text-orange-600" /> PDF Statement
                </Button>
                <Button
                  variant="outline"
                  size="sm"
        ...
        <CardContent>
          {!selectedCustomerId ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-3">
              <ReceiptText className="h-8 w-8 opacity-40" /><p className="text-sm">No pending invoices for this customer</p>
            </div>
          ) : (
            <>
              {/* Desktop Table */}
                        ...
                        <TableRow key={inv.id} className={cn(isSelected && "bg-orange-50/40")}>
                          ...
                          <TableCell className="text-center">
                            <Button
                              ...
                              className="h-7 px-2 text-xs text-orange-600 hover:text-orange-800 hover:bg-orange-100/50 gap-1"
                              title="View Payment History"
                            >
                              <History className="w-3.5 h-3.5" />
                              {paymentCount > 0 ? (
                                <span className="font-semibold text-orange-700">{paymentCount}</span>
                              ) : (
                                <span>0</span>
                              )}
                            </Button>
                          ...
                          <TableCell className="text-right">
                            <Input type="number" min="0" step="0.01" max={inv.balance} disabled={!isSelected}
                              value={s?.settleAmount || ""} onChange={(e) => updateSettleAmount(inv.id, parseFloat(e.target.value) || 0, inv.balance)}
                              className="w-36 text-right ml-auto" placeholder="0.00" />
                          </TableCell>
                        </TableRow>
                      );
                    })}
              ...
              <div className="md:hidden space-y-3">
                {pendingInvoices.map((inv) => {
                  ...
                  return (
                    <div key={inv.id} className={cn("border rounded-lg p-4 space-y-3 transition-colors", isSelected ? "border-orange-400 bg-orange-50" : "bg-card")}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {inv.manualInvoiceNo && <p className="text-xs font-mono text-muted-foreground">{inv.manualInvoiceNo}</p>}
                      ...
                      {isSelected && (
                        <div className="space-y-1">
                          ...
                          <Input type="number" min="0" step="0.01" max={inv.balance} value={s?.settleAmount || ""} onChange={(e) => updateSettleAmount(inv.id, parseFloat(e.target.value) || 0, inv.balance)} placeholder="0.00" className="text-right" />
                        </div>
                      )}
                    </div>
                  );
      ...
      {selectedCustomerId && totalAmount > 0 && (
        <Card className={cn("border-2", remaining < 0 ? "border-destructive/50 bg-destructive/5" : remaining === 0 ? "border-green-500/50 bg-green-50/50" : "border-orange-200")}>
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex flex-wrap gap-6">
                ...
                <div className="space-y-0.5"><p className="text-xs text-muted-foreground uppercase tracking-wide">Allocated</p><p className="text-xl font-bold text-orange-500">{formatCurrency(totalAllocated)}</p></div>
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                <Button onClick={handleSubmit} disabled={isSubmitting || remaining < 0 || Object.values(settlements).filter((s) => s.selected && s.settleAmount > 0).length === 0} className="flex-1 sm:flex-none gap-2 bg-orange-500 hover:bg-orange-600">
                  {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" />Saving…</> : <><BanknoteIcon className="h-4 w-4" />Settle Invoices</>}
                </Button>
              </div>
            </div>
            {remaining < 0 && <p className="mt-3 text-sm text-destructive flex items-center gap-1"><AlertCircle className="h-4 w-4" />Allocated amount exceeds payment amount. Please adjust.</p>}
        ...
        <DialogContent className="max-w-md">
          ...
          {historyModalInvoice && (
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-3 gap-2 p-3 bg-orange-50/50 border border-orange-100 rounded-lg text-xs">
                <div>
