...
interface OverdueInvoice {
  ...
  invoiceNo: string;
  customerName: string;
  shopName: string; // Assuming shopName might be part of customer data or separate
  phone: string;
  dueDate: string;
  amount: number; // This is the due amount
  daysOverdue: number;
  status: string;
}
    ...
    return rawInvoices
      .map((inv: any) => {
        ...
        const dueDate = inv.dueDate
          ...
          : new Date(new Date(inv.date || inv.createdAt).setDate(new Date(inv.date || inv.createdAt).getDate() + 30));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          {/* Updated Title Color for Orange Agency Context */}
