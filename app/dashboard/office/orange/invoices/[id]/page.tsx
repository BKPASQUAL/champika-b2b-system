...
// Import print utils relative to this module
...
export default function OrangeViewInvoicePage({
  params,
}: {
  ...
  const fetchInvoice = async (silent = false) => {
    ...
    try {
      ...
      if (!silent) router.push("/dashboard/office/orange/invoices");
    } finally {
  ...
  const handleSharePdf = () =>
    shareInvoice(id, "orange", invoice?.invoiceNo || "", setSharing);
                          ...
                          <TableRow key={pay.id} className={isReturned ? "bg-red-50/40 hover:bg-red-50/60" : "hover:bg-emerald-50/10"}>
                            ...
                            <TableCell className="text-sm text-muted-foreground">
                              <div className="flex flex-col gap-1">
                                {pay.receipt_number ? (
                                  <Badge variant="outline" className="font-mono text-[11px] font-bold text-orange-800 bg-orange-50 border-orange-200 w-fit">
