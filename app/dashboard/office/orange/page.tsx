...
export default function OrangeDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const invRes = await fetch(`/api/invoices?businessId=${orangeId}`);
      const invoices = invRes.ok ? await invRes.json() : [];

      // 2. Fetch Purchases (Supplier Payables)
      const purRes = await fetch(`/api/purchases?businessId=${orangeId}`);
      const purchases = purRes.ok ? await purRes.json() : [];

      // 3. Fetch Inventory (Stock Levels)
      const stockRes = await fetch(`/api/inventory?businessId=${orangeId}`);
      ...
      // ✅ FIXED: Added check for inv.date existence
      ...
      // C. Supplier Due (Payables to Orel)
