...
// Import print utils relative to Wireman module
...
export default function WiremanViewInvoicePage({
  ...
  const fetchInvoice = async (silent = false) => {
    ...
    try {
      ...
      if (!silent) router.push("/dashboard/office/wireman/invoices");
    } finally {
  ...
  const handleSharePdf = () =>
    shareInvoice(id, "wireman", invoice?.invoiceNo || "", setSharing);
  ...
  // Gross Total acts as the Invoice Total BEFORE Returns
      ...
      <div className="mx-auto space-y-8">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <Button
                ...
                onClick={() => {
                  if (fromProduct) {
                    router.push(`/dashboard/office/wireman/products/${fromProduct}`);
                  } else {
                    router.back();
                  }
                }}
                    ...
                    <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:w-0.5 before:bg-slate-200">
                      {historyLogs.map((log) => (
                        <div key={log.id} className="relative flex gap-4">
                          <div
                            className="absolute left-0 mt-1.5 w-2.5 h-2.5 rounded-full bg-white border-2 border-red-500 z-10"
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Returns & Adjustments (Themed Red for Wireman) */}
            {returns.length > 0 && (
              <Card className="shadow-sm border-l-4 border-l-red-500 overflow-hidden">
                <CardHeader className="bg-red-50/50 border-b py-4">
                  <div className="flex justify-between items-center">
                ...
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody>
                      {returns.map((ret) => (
                        <TableRow key={ret.id} className="hover:bg-red-50/10">
                ...
                <CardFooter className="bg-red-50/30 border-t py-3 flex justify-between">
                          ...
                          >
                            ...
                            <TableCell className="text-sm text-muted-foreground">
                              <div className="flex flex-col gap-1">
                                {pay.receipt_number ? (
                                  <Badge variant="outline" className="font-mono text-[11px] font-bold text-red-800 bg-red-50 border-red-200 w-fit">
