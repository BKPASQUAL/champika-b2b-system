...
export default function CreateWiremanDamageReportPage() {
  ...
  const fetchLocations = async () => {
    try {
      const res = await fetch(
        `/api/settings/locations?businessId=${BUSINESS_IDS.WIREMAN_AGENCY}`
      );
  ...
  const fetchLocationStock = async () => {
    ...
    try {
      const res = await fetch(
        `/api/inventory/${selectedLocationId}?includeAll=true&businessId=${BUSINESS_IDS.WIREMAN_AGENCY}`
      );
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-red-950">
              Report New Damage (Wireman)
            </h1>
          </div>
          <p className="text-muted-foreground mt-1 ml-8">
            Mark inventory as damaged in Wireman Agency locations.
          </p>
        </div>
        <Button
          variant="destructive"
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <MapPin className="w-4 h-4 text-red-600" /> Location & Context
              </CardTitle>
            </CardHeader>
          ...
          <Card className={!selectedLocationId ? "opacity-60 pointer-events-none" : ""}>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Package className="w-4 h-4 text-red-600" /> Select Items
              </CardTitle>
            </CardHeader>
            <CardContent>
              {stockLoading ? (
          ...
          <Card className="h-full flex flex-col">
            <CardHeader className="border-b pb-3">
              <div className="flex justify-between items-center">
                <CardTitle className="flex items-center gap-2 text-base">
                  <ClipboardList className="w-4 h-4 text-red-600" /> Damage List
                </CardTitle>
