...
export default function WiremanStockAdjustmentPage() {
  ...
  const handleSaveAll = () => {
    ...
    try {
      ...
      // Append zero entries for unlisted products if toggle is on
      if (zeroOutUnlisted) {
      ...
      const res = await fetch("/api/inventory/adjust", {
        ...
        body: JSON.stringify({
          ...
          reason: zeroOutUnlisted
            ? "Full Stock Count — unlisted items zeroed (Wireman Portal)"
            : "Manual Stock Adjustment (Wireman Portal)",
        ...
        <Card className="md:col-span-1 h-fit border-red-100">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex rounded-md border overflow-hidden text-sm font-medium">
              <button
                type="button"
                className={cn("flex-1 py-2 transition-colors", addMode === "set" ? "bg-red-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70")}
              ...
              <button
                type="button"
                className={cn("flex-1 py-2 transition-colors", addMode === "add" ? "bg-red-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/70")}
